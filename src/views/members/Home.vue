...
<div class="relative inline-block text-left">
  ...
  <div v-show="showReactMenu[index]" class="z-10 origin-top-left absolute left-0 mt-2 w-40 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5">
    <div class="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
      ...
      <button v-show="$store.getters.loggedInUserId === post.authorId" @click="showPublish = true, showReactMenu[index] = false, setupEdit(post._id, post.text)" class="w-full flex items-center block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" role="menuitem">
      ...
      <button v-show="$store.getters.loggedInUserId === post.authorId" @click="handleDelete(post._id)" class="w-full flex items-center block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-gray-900" role="menuitem">
...
export default {
  ...
  methods: {
    ...
    async handleDelete(id) {
      ...
      query = `mutation {
        messageRemoveMany(filter: { messageId: "${id}" }) {
          numAffected
        }
      }`;
      await axios.post(process.env.VUE_APP_API || apiUrl, { query });
